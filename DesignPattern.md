...
备忘录模式又称快照、Snapshot，是一种行为型设计模式，允许在不暴露对象实现细节的情况下保存和回复对象之前的状态。



### Intent

思考，如果我们正在开发一款编辑器程序，需要为其实现 "撤销" 功能，我们可以选择最简单直接的方式去实现：程序在执行任何操作前会记录所有的对象状态，并将其保存下来。此后用户需要撤销某个操作时，程序从历史状态中获取最近的快照，然后使用它来恢复所有对象的状态。

但是这种方法存在问题，因为生成快照的类需要获取所有的数据，破坏了封装性。



### Solution

刚刚提到的问题主要是因为违背了类的封装性导致的。一些对象试图超过其职责范围，在执行时需要获取其他对象的私有数据，而不是让这些对象完成工作。

备忘录模式将创建状态快照（Snapshot）的工作委派给实际状态的拥有者 Originator 对象。这样其他对象就不需要从 "外部" 复制编辑器状态了，编辑器类拥有其状态的完全访问权，因此可以自行生成快照。

同时模式建议将对象状态的副本存储在一个名为备忘录（Memento）的特殊对象中。除了创建备忘录的对象外，任何对象都不能访问备忘录的内容。其他对象必须使用受限接口与备忘录进行交互，它们可以获取快照的元数据（创建时间和操作名称等等），但不能获取快照中原始对象的状态。
...
备忘录模式的经典实现依赖于很多流行编程语言的嵌套类机制：

主要有以下几种角色：

（1）原发器（Originator）类可以生成自身状态的快照，也可以在需要时通过快照恢复自身状态；

（2）备忘录（Memento）是原发器状态快照的值对象（value object）。通常做法是将备忘录设置为不可变的，并通过构造函数一次性传递数据；
...
（2）当直接访问对象的成员变量、getter 或 setter 将导致封装被破坏时，可以使用该模式；
...
- 可以同时使用 Command 和 Memento 来实现 "撤销" 功能。此种情况下，命令用于对目标对象执行各种不同的操作，备忘录用来保存一条命令执行前该对象的状态；
- 可以同时使用 Memento 和 Iterator 来获取当前迭代器的状态，并且在需要的时候进行回滚；
- 有时候 Prototype 可以作为 Memento 的一个简化版本，其条件是在历史记录中存储的对象的状态比较简单，不需要链接其他外部资源，或者链接可以很方便的重建。



## Observer

观察者模式又称：事件订阅者、监听者、Event-Subscriber、Listener、Observer；

观察者是一种行为型设计模式，允许你定义一种订阅机制，可在对象事件发生时通知多个 "观察" 该对象的其他对象。
